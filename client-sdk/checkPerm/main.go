...
// ... [此处省略 newGrpcConnection, loadCertificate, newIdentity, newSign, handleError 函数] ...
...
func main() {
	...
	// --- 修改部分：从文件中读取 User2 的哈希 ID ---
	uidPath := "../register/user_2_id.txt"
	uidStr, err := readUserIDFromFile(uidPath)
	if err != nil {
		log.Fatalf("无法从文件 %s 读取 UserID: %v", uidPath, err)
	}
	fmt.Printf("正在对用户 (User2) 进行权限检查，哈希 ID: %s\n", uidStr)

	// 读取 user_2 私钥
	priv, err := loadRSAPrivateKeyFromPEMFile("../register/user_2_private_key.pem")
	if err != nil {
		log.Fatalf("加载 user_2 私钥失败: %v", err)
	}

	// (1) 对 uidStr||cid 签名 (这里的 uidStr 是哈希串)
	...
	// (2) 调用合约 CheckPerm(signature, operation, uid, cid)
	fmt.Println("提交 CheckPerm 交易中...")
	decisionBytes, err := contract.SubmitTransaction(
		"CheckPerm",
		sigB64,    // 签名
		operation, // "download"
		uidStr,    // 用户哈希 ID
	...
	if err != nil {
		handleError(err)
		log.Fatalf("CheckPerm 失败: %v", err)
	}

	fmt.Printf("CheckPerm 返回结果：%s\n", string(decisionBytes))
	fmt.Printf("总耗时 %.3f ms\n", float64(time.Since(start).Milliseconds()))
}
